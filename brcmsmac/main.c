		       ...
		       bool suspend)
{
	...
	//goto pass;
	if (1)
		params = & (struct ieee80211_tx_queue_params) {
			.aifs = 1,
			.cw_min = 3,
			.cw_max = 7,
			.txop = 16,
	...
	if (acp_shm.aifs < EDCF_AIFSN_MIN
		...
		printk("brcmsmac: setting wme params for ac %d\n", aci);
		printk("... aifs: %d; cwmin: %d; cwmax: %d; txop: %d;\n",
			acp_shm.aifs, acp_shm.cwmin, acp_shm.cwmax,
			acp_shm.txop);
...
#define ANTCFG_NONE 0xFF
	...
	if (ac<4) wlc->macq_stats[ac]++;
	else printk("unexpected ac>4\n");
	//printk("tx prio %d; ac %d\n", (u8)p->priority, ac);
...
void brcms_c_init(struct brcms_c_info *wlc, bool mute_tx)
{
	...
	printk("%s: d11.ifs_ctl = %X\n", __func__,
	       bcma_read16(core, D11REGOFFS(ifs_ctl)));
