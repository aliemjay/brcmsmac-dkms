...
static int brcms_debugfs_macstat_read(struct seq_file *s, void *data)
{
	...
	uint n_beacon_updates;
	int i;

	spin_lock_bh(&wl->lock);
	stats = *(drvr->wlc->core->macstat_snapshot);
	n_beacon_updates = wl->wlc->n_beacon_updates;
	...
	seq_printf(s, "n_beacon_updates: %d\n", n_beacon_updates);
	for (i=0; i<4; i++)
		seq_printf(s, "macq_stats for %d: %d\n",
			   i, wl->wlc->macq_stats[i]);
	seq_printf(s, "ntx: %d\n", wl->wlc->stats_ntx);
	seq_printf(s, "nampdu: %d\n", wl->wlc->stats_nampdu);
	return 0;
}
