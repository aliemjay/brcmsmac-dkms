...
struct scb {
	...
	uint fragresid[AMPDU_MAX_SCB_TID];/* #bytes unused in frag buffer per prio */

	u16 seqctl[AMPDU_MAX_SCB_TID];	/* seqctl of last received frame (for dups) */
	/* seqctl of last received frame (for dups) for non-QoS data and
	 * management */
	u16 seqctl_nonqos;
	u16 seqnum[AMPDU_MAX_SCB_TID];/* WME: driver maintained sw seqnum per priority */
